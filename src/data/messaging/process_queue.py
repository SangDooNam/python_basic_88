...
def parse_message(text):
    ...
    lst = []
    message_dict = {}
    with open(QUEUE_ROOT+'/'+text, 'r') as message:
...
def create_user_directories(user):
    ...
    path_user = '/' + user 
    path_read = path_user + '/' + 'read'
    path_unread = path_user + '/' + 'unread'
    os.makedirs(USERS_ROOT+path_user, exist_ok=True)
    os.makedirs(USERS_ROOT+path_read, exist_ok=True)
    os.makedirs(USERS_ROOT+path_unread, exist_ok=True)
...
def send_message(message):
    ...
    if random.random() > 0.5:
        ...
        status = True
        with open(USERS_ROOT+slash+to+slash+unread+file_name, 'w') as sending_msg:
            sending_msg.write(msg)
        
        
        with open(ROOT+'/'+'messages.log', 'a') as log:
...
def move_to_sent(message):
    ...
    slash = '/'
    sent = 'sent'
    path_sent = QUEUE_ROOT+slash+sent
    os.makedirs(path_sent, exist_ok=True)
    os.rename(QUEUE_ROOT+slash+message, path_sent+slash+message)
...
def notification(status, message):
    ...
    title = title.replace('\n', '').strip()
    path_ok = ROOT + '/' + 'templates' + '/' + 'ok.txt'
    path_ko = ROOT + '/' + 'templates' + '/' + 'ko.txt'
    path_sender = USERS_ROOT + '/' + sender + '/' + 'unread'
    file_name = create_file_name()
